...
var render = {
    ...
    getArmyTable:function () {
        ...
        var headerCell = '<th sitr_tip="{0}" style="display:none;background:url(\'{1}\')  no-repeat center center;background-size: 26px auto;" colspan="2" \n    class="army_unit_{2} {3}">&nbsp;</th>\n\n';
            ...
            for (var category in Constant.unitOrder) {
                $.each(Constant.unitOrder[category], function (index, value) {
                    headerCells += Utils.format(headerCell, [database.getGlobalData.getLocalisedString(value),
                                                 getImage(value), value,(index == 0) ? 'lf' : 'lfdash'])
                })
            }
            ...
            $.each(Constant.Buildings, function(key, value){
                $.each(city.getBuildingsFromName(value), function (index, building) {
                    this.getBuildingsRow(city).find('.building.' + building.getName + index +' a')
                        .removeClass('upgradable')
                        .addClass(building.isUpgradable ? 'upgradable SITR_clickable' : 'SITR_clickable');
                }.bind(render))
            })
        }
    },
        ...
        get _getSatisfactionData() {
            ...
            if (this.getBuildingFromName(Constant.Buildings.TAVERN)) {
                ...
                var consumption = Math.floor(
                    this.getResource(Constant.Resources.WINE).getConsumption * (100/( 100 - (this.getBuildingFromName(Constant.Buildings.VINEYARD)?this.getBuildingFromName(Constant.Buildings.VINEYARD).getLevel:0))))
